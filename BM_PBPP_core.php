...
function bte_opp_update_old_post($oldest_post) {
	...
	bte_opp_tweet($post->post_title." ".$shorturl." #PBPP");
...
function bte_opp_get_short_url($url) {
	...
	$c=new xmlrpc_client(BM_PBPP_XMLRPC, BM_PBPP_XMLRPC_URI, 80);
	if (false) {
		$c->setDebug(1);
	}
	$r=&$c->send($f);
...
function bte_opp_sendXmlrpc($server, $permalink) {
	...
	$client->timeout = 3;
	$client->useragent .= ' -- WordPress/PBPP';
...
function bte_opp_tweet($tweet) {
	...
	$snoop = new Snoopy;
	$snoop->agent = 'HOW TO MAKE MONEY ONLINE http://111waystomakemoney.com/';
	$snoop->rawheaders = array(
		'X-Twitter-Client' => 'Powerfull Blog Post Promoter'
	...
	$snoop->pass = $pass;
	$snoop->submit(
		BM_RT_API_POST_STATUS
		, array(
			'status' => $tweet
			, 'source' => 'Powerfull Blog Post Promoter'
		)
	);
...
function bte_opp_the_content($content) {
	...
	if (isset($origPubDate) && $origPubDate!='') {
		if ($showPub || $givecredit) {
			...
			if ($givecredit) {
					$dateline.='Republished by  <a href="http://111waystomakemoney.com/powerful-blog-post-promoter/">Blog Post Promoter</a>';
			}
			$dateline.='</small></p>';
		}
	}

<?php
/*  Copyright 2008-2009  Sharath (email : kevin@blogtrafficexchange.com)
...
*/
require_once('PowerfulBlogPostPromoter.php');
require_once('BM_PBPP_core.php');

function bte_opp_head_admin() {
	wp_enqueue_script('jquery-ui-tabs');
	$home = get_settings('siteurl');
	$base = '/'.end(explode('/', str_replace(array('\\','/BM_PBPP_admin.php'),array('/',''),__FILE__)));
...
function bte_opp_options() {	 	
	$message = null;
	$message_updated = __("Powerfull Blog Post Promoter Options Updated.", 'bte_old_post_promoter');
	if (!empty($_POST['bte_opp_action'])) {
		...
		print('
			<div id="message" class="updated fade">
				<p>'.__('PBPP Options Updated.', 'OldPostPromoter').'</p>
			</div>');
	}
	$omitCats = get_option('bte_opp_omit_cats');
	if (!isset($omitCats)) {
		$omitCats = BM_PBPP_OMIT_CATS;
	}
	$ageLimit = get_option('bte_opp_age_limit');
	if (!isset($ageLimit)) {
		$ageLimit = BM_PBPP_AGE_LIMIT;
	}
	...
	print('
			<div class="wrap">
				<h2>'.__('Powerfull Blog Post Promoter by', 'PowerfulblogPostPromoter').' <a href="http://111waystomakemoney.com">Sharath</a></h2>


<h1 style="text-align: left">For Instructions Visit:<a href="http://111waystomakemoney.com/powerful-blog-post-promoter/" target="_blank">Plugin Page</a></h1>

				<form id="bte_opp" name="bte_oldpostpromoter" action="'.get_bloginfo('wpurl').'/wp-admin/options-general.php?page=BM_PBPP_admin.php" method="post">
					...
					<fieldset class="options">
						...
						<div class="option">
							...
							<select name="bte_opp_interval" id="bte_opp_interval">
									<option value="'.BM_PBPP_1_HOUR.'" '.bte_opp_optionselected(BM_PBPP_1_HOUR,$interval).'>'.__('1 Hour', 'OldPostPromoter').'</option>
									<option value="'.BM_PBPP_4_HOURS.'" '.bte_opp_optionselected(BM_PBPP_4_HOURS,$interval).'>'.__('4 Hours', 'OldPostPromoter').'</option>
									<option value="'.BM_PBPP_6_HOURS.'" '.bte_opp_optionselected(BM_PBPP_6_HOURS,$interval).'>'.__('6 Hours', 'OldPostPromoter').'</option>
									<option value="'.BM_PBPP_12_HOURS.'" '.bte_opp_optionselected(BM_PBPP_12_HOURS,$interval).'>'.__('12 Hours', 'OldPostPromoter').'</option>
									<option value="'.BM_PBPP_24_HOURS.'" '.bte_opp_optionselected(BM_PBPP_24_HOURS,$interval).'>'.__('24 Hours (1 day)', 'OldPostPromoter').'</option>
									<option value="'.BM_PBPP_48_HOURS.'" '.bte_opp_optionselected(BM_PBPP_48_HOURS,$interval).'>'.__('48 Hours (2 days)', 'OldPostPromoter').'</option>
									<option value="'.BM_PBPP_72_HOURS.'" '.bte_opp_optionselected(BM_PBPP_72_HOURS,$interval).'>'.__('72 Hours (3 days)', 'OldPostPromoter').'</option>
									<option value="'.BM_PBPP_168_HOURS.'" '.bte_opp_optionselected(BM_PBPP_168_HOURS,$interval).'>'.__('168 Hours (7 days)', 'OldPostPromoter').'</option>
							</select>
						</div>
						<div class="option">
							...
							<select name="bte_opp_interval_slop" id="bte_opp_interval_slop">
									<option value="'.BM_PBPP_1_HOUR.'" '.bte_opp_optionselected(BM_PBPP_1_HOUR,$slop).'>'.__('Upto 1 Hour', 'OldPostPromoter').'</option>
									<option value="'.BM_PBPP_4_HOURS.'" '.bte_opp_optionselected(BM_PBPP_4_HOURS,$slop).'>'.__('Upto 4 Hours', 'OldPostPromoter').'</option>
									<option value="'.BM_PBPP_6_HOURS.'" '.bte_opp_optionselected(BM_PBPP_6_HOURS,$slop).'>'.__('Upto 6 Hours', 'OldPostPromoter').'</option>
									<option value="'.BM_PBPP_12_HOURS.'" '.bte_opp_optionselected(BM_PBPP_12_HOURS,$slop).'>'.__('Upto 12 Hours', 'OldPostPromoter').'</option>
									<option value="'.BM_PBPP_24_HOURS.'" '.bte_opp_optionselected(BM_PBPP_24_HOURS,$slop).'>'.__('Upto 24 Hours (1 day)', 'OldPostPromoter').'</option>
							</select>
						</div>
						<div class="option">
	...
	print('				    		</ul>
					...
					<p class="submit">
						<input type="submit" name="submit" value="'.__('Update PBPP Options', 'OldPostPromoter').'" />
					</p>
<h2 style="text-align: left">If u like the plugin please Donate:<a href="http://111waystomakemoney.com/donate/" target="_blank">Plugin Donation Page</a></h2>
						<div class="option">
							<h4>Other Sharath <a href="http://111waystomakemoney.com/wordpress-plugins/">Wordpress Plugins</a></h4>
							<ul>
							<li><a href="http://111waystomakemoney.com/wordpress-plugins/powerful-social-bookmarking-wordpress-plugin/">Powerfull Social Bookmarking</a></li>
							<li><a href="http://111waystomakemoney.com/wordpress-plugins/powerful-blog-copyright/">Powerfull Blog Copyright</a></li>
							<li><a href="http://111waystomakemoney.com/powerful-blog-post-promoter/">Powerfull Blog Post Promoter</a></li>
							<li><a href="http://111waystomakemoney.com/wordpress-plugins/related-websites/">Powerfull Related Websites</a></li>
							<li><a href="http://111waystomakemoney.com/wordpress-plugins/related-posts/">Powerfull Related Posts</a></li>
							</ul>
						</div>
				</form>' );
...
function bte_opp_options_setup() {	
	add_options_page('OldPostPromoter', 'Powerfull Blog Post Promoter', 10, basename(__FILE__), 'bte_opp_options');
}
